...
import { TIPO_CLIENTE_CONFIG, STATUS_CLIENTE_CONFIG, CATALOGO_SERVICOS } from '@/lib/clientes/constants'
...
export function ClienteCard({ cliente, contratos, onClick }: Props) {
  ...
  return (
    <div
      onClick={onClick}
      className={`bg-white rounded-xl border-l-4 border border-slate-200 p-4 cursor-pointer hover:shadow-md transition-all group ${
          ...
          {Array.from({ length: 10 }).map((_, i) => (
            <div
              ...
              className={`h-1.5 flex-1 rounded-full ${
                i < cliente.nps! ? (cliente.nps! >= 9 ? 'bg-emerald-400' : cliente.nps! >= 7 ? 'bg-amber-400' : 'bg-red-400') : 'bg-slate-100'
              }`}
            />
          ))}
